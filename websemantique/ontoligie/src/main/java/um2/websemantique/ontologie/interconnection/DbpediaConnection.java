...
public class DbpediaConnection {
	...
	/**
	 * Multiple words, complete phrase,in start of phrase. Select distinct
	 * values of subjects and objects, where subjects have labels that start
	 * with the exact searched keywords. The subject is not from:
	 * http://dbpedia.org/resource/Category http://dbpedia.org/resource/List
	...
	public static Resource executeSPARQLToDbpedia(String word) {
		Query query = null;
		try {
			query = QueryFactory.create (createQueryForDbpedia (word));
	...
	public static String createQueryForDbpedia(String word) {
		String result = "SELECT DISTINCT ?s ?o" + " WHERE {"
		...
		result += aux + "(regex(str(?o), \'^" + words[words.length - 1]
				+ "\', 'i')) ).";
